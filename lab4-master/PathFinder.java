...
public class PathFinder<Node> {
    ...
    public Result searchAstar(Node start, Node goal) {
         ...
         * Change below this comment  *
         ******************************/
        Queue<PQEntry> pqueue = new PriorityQueue<>(Comparator.comparingDouble(entry -> entry.predictedCost));
        HashSet<Node> visitedNodes = new HashSet<>();
        ...
        while(!pqueue.isEmpty()) {
            ...
            if (!visitedNodes.contains(entry.node)) {
                ...
                for (DirectedEdge<Node> edge : graph.outgoingEdges(entry.node)) {
                    double costToNext = entry.costToHere + edge.weight();
                    double predictedTotalCost = graph.guessCost(edge.to(), goal) + costToNext;
                    pqueue.add(new PQEntry(edge.to(), costToNext, entry, predictedTotalCost));
                }
                visitedNodes.add(entry.node);
            }
        }
    ...
    private class PQEntry {
        ...
        PQEntry(Node n, double c, PQEntry bp, double d) {
            node = n;
            costToHere = c;
            backPointer = bp;
            predictedCost = d;
